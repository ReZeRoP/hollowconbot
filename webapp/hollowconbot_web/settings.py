"""Django settings for HollowConBot Web Panel."""

import os
from pathlib import Path

BASE_DIR = Path(__file__).resolve().parent.parent
BOT_DIR = BASE_DIR.parent  # /opt/HollowConBot
...
ROOT_URLCONF = "hollowconbot_web.urls"
...
WSGI_APPLICATION = "hollowconbot_web.wsgi.application"

# HollowConBot shares the same SQLite database
...
# Include the configured port when it is non-default — otherwise POSTs from
# https://domain:4043/... fail CSRF after the Mini App loads.
_web_domain = os.environ.get("WEB_DOMAIN", "").strip()
_web_port = os.environ.get("WEB_PORT", "").strip()
...
if _web_domain:
    ...
    if _web_port and _web_port not in ("80", "443"):
        _origins.add(f"https://{_web_domain}:{_web_port}")
        _origins.add(f"http://{_web_domain}:{_web_port}")
